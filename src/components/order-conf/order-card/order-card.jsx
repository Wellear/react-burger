...
import { ITEM_DISPLAY, MAX_ITEMS } from "../../../utils/constns";
...
const OrderCard = ({ order }) => {
  ...
  const orderLength = orderIngredients.length;
  ...
  const displayedItems =
    orderLength > 6 ? orderIngredients.slice(0, 6) : orderIngredients;
  ...
  return (
    <div className={`${styles.container} pt-6 pb-6 pl-6 pr-6 mr-2`}>
      ...
      <div className={styles.price}>
        ...
        {orderIngredients && orderLength > MAX_ITEMS && (
          <p
            className={`text text_type_main-default ${styles.cover}`}
            style={{ left: (MAX_ITEMS - 1) * ITEM_DISPLAY }}
          >{`+${orderLength - MAX_ITEMS}`}</p>
        )}
        <div className={styles.priceContainer}>
          <p className={`${styles.sum} text text_type_digits-default pr-2`}>
            {orderTotal}
          </p>
